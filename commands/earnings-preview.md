...
> 两者是**两个独立方法**：`earnings` 读 dim 21 的缓存结果；`earnings-preview` 是 Tier-1 按需计算，不在任何维度里。

适配 A股 / 港股 / 美股个股，行业观察指标与隐含波动按市场自适应。

## 工作流

1. 采集数据（22 维）：`uzi <ticker> --stage1` 产出缓存，一致预期优先取 `6_research`。
2. 按需计算预览：
   ```bash
   uzi <ticker> --method earnings-preview
   ```
