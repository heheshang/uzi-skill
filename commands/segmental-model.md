...
22 维框架里，DCF / 3-statement / IC Memo 都是 **top-down**——从整体营收 × 单一增速假设推未来。遇到业务结构在转型的公司就失真：
...
先跑过 `uzi <ticker> --stage1` 拿到缓存，再执行：

```bash
uzi <ticker> --segmental discover
```

从 `.cache/<ticker>/raw_data.json` 读 `5_chain.breakdown_top` + `1_financials.revenue_history` + `15_events`，产出 `.cache/<ticker>/segmental_skeleton.json`：
...
在骨架的每个 `segments[*]` 上补齐 `drivers` / `thesis_tag` / 三个 CAGR，写入 `.cache/<ticker>/segmental_model.json`，然后：

```bash
uzi <ticker> --segmental validate
```

校验结果写入 `.cache/<ticker>/segmental_validation.json`（含 `passed` / `errors` / `warnings` / `summary`），**exit≠0 = 未通过**。校验规则：
- 🔴 sum(segment.latest_revenue_yi) 必须对账回 `1_financials.revenue_history` 末值 ±10%
- 🔴 每个 segment bull ≥ base ≥ bear 单调
- 🟡 Base 情景 3 年总增速 > 100% 需要明确收购/新业务 note
- 🟡 每个 segment 必须有 ≥ 1 个 driver，且 `thesis_tag` 非空

不过不发。

### Step 5 · 进报告

报告组装阶段检测到 `.cache/<ticker>/segmental_model.json` 存在 → 在 DCF 块下面加「分业务 Build-Up」块 · 用 top-down DCF 的 FCF projection vs bottom-up segmental 的 revenue projection 做交叉校验。
...
这是 `--method` 21 个 NAME 之外的**独立补充方法**，和 DCF 互补，不是替代。两个结论应该对得上：
