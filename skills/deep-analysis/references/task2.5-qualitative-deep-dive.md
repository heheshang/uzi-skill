...
2. 用 CDP 浏览器兜底（`uzi_data::browser::fallback`）打开以下 URL 抓取最新政策原文：
...
2. 用 CDP 浏览器兜底（`uzi_data::browser::fallback`）打开：
...
- 先走 CDP 浏览器兜底（`uzi_data::browser::fallback`）/ WebSearch
...
直接复制到 sub-agent prompt 里让它用 CDP 浏览器兜底打开（`uzi_data::browser::fallback`）。
...
```
A 股：
https://www.iwencai.com/                           (问财 NLP · 需 CDP 浏览器；'同行业 市值>100亿' 类查询)
https://stockpage.10jqka.com.cn/                   (同花顺 F10 · 需 CDP 浏览器；行业/概念板块 + 同行)

港股：
https://www.aastocks.com/sc/stocks/analysis/industry/  (AASTOCKS · 港股按行业列表 · 需 CDP 浏览器)
```

### Dim 7 · 行业（v2.5 扩充）
```
https://xueqiu.com/S/{code}/F10/industry           (雪球 F10 行业信息 · 需 CDP 浏览器)
