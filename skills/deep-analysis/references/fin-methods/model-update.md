...
| 参数 | 说明 |
|---|---|
| `features` | `uzi_features::stock_features :: extract_features` 输出 |
| `raw_data` | 原始 22 维（仅用于取公司名/code） |
| `updates` | 新假设对象（见下）。未提供 → 演示模式，从 features 推断「最新 vs 上期」 |
| `dcf_result` | `uzi_models::fin_models :: compute_dcf` 输出 → 算内在价值 delta（不传则标「未提供」） |
| `comps_result` | `uzi_models::fin_models :: build_comps_table` 输出 → 算隐含价 delta |
...
为保持纯函数 + 无副作用，本模块**不回灌** `uzi_models::fin_models :: compute_dcf`，而用一阶弹性近似重定价：
...
> 这是「快速影响估算」。要精确数字时，把 `updates` 转成 `compute_dcf(features, Some(&assumptions))`
> 重跑一遍 DCF，再把两个 DCF 结果对比（精度高于本模块的一阶近似）。
...
- 上游：`uzi_models::fin_models :: compute_dcf`、`uzi_models::fin_models :: build_comps_table`（dim 20 内产出）提供 `dcf_result` / `comps_result`。
- 平行：`uzi_models::research_workflow :: build_earnings_analysis` 做 beat/miss 定性，本模块做定量重定价——
  财报后通常先看 earnings 超预期与否，再把新数字灌进模型看目标价怎么动。
- 下游：`uzi_models::research_workflow :: build_thesis_tracker` 跟踪支柱状态；本模块的 `thesis_impact` 是「假设变动 → 支柱方向」的瞬时映射。
