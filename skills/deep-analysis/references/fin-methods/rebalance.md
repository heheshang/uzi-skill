...
pub fn build_rebalance(
    holdings: &Value,               // [{ticker, weight, market, industry?, value?, price?}]
    targets: Option<&Value>,        // {ticker: 目标权重}；None → 等权 1/N
    drift_threshold: f64,           // 漂移阈值，单位「百分点」，通常 5.0
) -> Value
...
实现细节（`STAMP_DUTY_A` / `STAMP_DUTY_HK` / `COMMISSION_A` / `COMMISSION_HK` 常量）：
...
`uzi_screen::portfolio`（`uzi --portfolio` 组合体检）的 holdings 结构是 `{ticker, weight, note}`。本模块在其上**扩展可选字段** `market / industry / value / price`，
