...
> **⚠️ v2.0 关键变化**：这是整个流水线里**最依赖 Claude 判断**的 Task。
> `uzi_pipeline::synthesis` 只给你一个 stub，**Claude 必须重写**关键字段。

读取 `dimensions.json` + `panel.json` + `raw_data.dimensions.20_valuation_models / 21_research_workflow / 22_deep_methods`，做**五件事**：

1. 算综合评分（`uzi_pipeline::synthesis` 完成）
...
Stage 1（`uzi <ticker> --stage1`）会把以下预计算结果存进 `raw_data.json`，**你必须先读**：

```text
raw_data.json
└─ dimensions
   ├─ 20_valuation_models.data
   │   ├─ dcf              — WACC + FCF + 敏感性表 + safety_margin
   │   ├─ comps            — 同行分位 + 隐含价
   │   ├─ lbo              — IRR + MOIC + verdict
   │   ├─ three_statement  — 5 年预测
   │   └─ summary          — 汇总指标
   ├─ 21_research_workflow.data
   │   ├─ initiating_coverage — 评级 + 目标价 + 投资论点 + 风险
   │   ├─ earnings_analysis   — 超预期检测
   │   ├─ catalyst_calendar   — 真实事件 + 未来日程
   │   ├─ thesis_tracker      — 5 条支柱完好率
   │   └─ idea_screens        — 5 套 quant 命中情况
   └─ 22_deep_methods.data
       ├─ ic_memo             — 投委会建议 + 三情景 + 风险缓解
       ├─ competitive_analysis — Porter + BCG
       ├─ dd_checklist        — 5 工作流 21 项
       └─ value_creation_plan — EBITDA 桥
```

**Claude 的第一件事**：读这些字段，**审查默认假设对这只股是否合理**（详见 `task1.5-institutional-modeling.md`）。

默认假设固定在内嵌建模层 `uzi_models::fin_models`（`DEFAULT_STAGE1_GROWTH = 0.10`、
`DEFAULT_STAGE2_GROWTH = 0.05`、各 5 年），`compute_dim_20` 调用时**不传覆盖**，
`uzi` CLI 也没有传入自定义假设的入口。所以若默认假设明显不合理（比如半导体股用 stage1=10%）：

- **不要**编造一个"改了假设重跑后的 DCF 数字" —— 把它写进 `institutional_triangulation.conflict_note`
  作为"假设敏感点"，并在 `dim_commentary["20_valuation_models"]` 里点明
- 需要复用缓存重跑整条建模链（不重新联网抓取）时用 `uzi <ticker> --from-modeling`
- 假设变更的 before → after 增量由 `uzi_models::tier1::model_update` 记录
...
**自动选角**（`signal: skip` 的评委——射程外游资等——不参与选角）：
...
`punchline` 是最后进入报告 Great Divide 区的金句，要求**朋友圈传播力**：
- 长度 20-30 字
- 必须有冲突感
- 数据 + 情绪 二选一以上
- 例如：
...
```json
{
  ...
  "buy_zones": { ... },
  "dashboard": {
    "core_conclusion": "78 分，可以蹲一蹲。66 位大佬里 24 人看多，最看好的是段永平 92 分。",
    "data_perspective": {
