...
Task 1.5 在 Task 1 采集完原始数据后自动执行，计算 3 个新维度（dim 20 / 21 / 22），共涉及 17 种机构级分析方法。计算入口为 `uzi_models::compute`（`compute_dim_20/21/22`），底层模型在 `uzi_models::fin_models`、`uzi_models::research_workflow`、`uzi_models::deep_methods`。
...
```text
// uzi_models::fin_models 的 compute_dcf(features, assumptions)
// 半导体高成长
compute_dcf(features, assumptions = {
    "stage1_growth": 0.22,
    "stage2_growth": 0.12,
    "terminal_g": 0.03,
    "beta": 1.4,
...
1. 手动调用 `uzi_data::fetch::similar_stocks` 单独重跑
