...
### 股海贼王 (`ghzw`) · 淘股吧公开实盘 (匿名网名 · 2016-02 开贴)

> 本段为**风格提炼**，不逐字转载原帖发言（原始发言/交割单为本地数据，未入库）。
> 模拟其口吻时按下列方法论特征**原创撰写**台词，不照抄原文。

方法论关键词（用于把握 comment 的逻辑与语气）：
- **复盘三问**：为什么涨停 / 在板块什么地位 / 在大盘什么地位
- **接力纪律**：弱转强快速板才算超预期 · 盘面确认强度再下手 · 竞争板胜率低不满仓赌
- **选股**：只做当期最强主线 · 逻辑硬的低位票爆发力足
- **格局票**：能看三五倍空间就不恐高，把代表性标的当某个时代主题的"情绪载体"
- **心性/操守**：吃到目标点数（约 15%）就休息 · 拼的是心境 · 反复强调不跟单 · 不收费

**风格**: 真诚自省、复盘狂、短线接力口吻。常用词"主线 / 地位 / 弱转强 / 超预期 / 格局 / 情绪载体 / 盘面给答案"。
...
> 注：已对照本仓库名册 `crates/uzi-investors/src/data/investors.json`（66 位）逐条核对本文档全部 `id` —— 46 个独立人物段落（`### 名字 (\`id\`)`）+ 7 个席位类游资（`hu_jl` / `chengdu` / `sunan` / `ningbo_st` / `liuyi_zl` / `liu_sh` / `gu_bl`）均在名册内，无对不上的 `id`。名册中其余 13 位（`altman` / `andreessen` / `asness` / `burry` / `chamath` / `chanos` / `gerstner` / `gurley` / `jensen_huang` / `musk` / `naval` / `saylor` / `zhang_lei`）本文档未收录原话段落，生成 `comment` 时按其 persona 风格模拟。
