...
**本仓库是上游 [`wbh604/UZI-Skill`](https://github.com/wbh604/UZI-Skill) 的 Rust 重实现**：把原来一整套 Python 业务代码，重写成**单一二进制 `uzi`**（crate 工作区在 `crates/`）。行为与上游逐字节对齐，差分测试见 [`tools/golden/`](tools/golden/README.md)。

[安装](#安装) · [用法](#用法) · [三档深度](#-三档思考深度v2103-新增) · [评审团](#-66-位评审团) · [Serenity 🆕](#-i-组--serenity--ai-卡位瓶颈猎手) · [机构方法](#-22-种机构级方法) · [自查 gate](#-机械级自查-gatev29-起) · [报告截图](#-报告长什么样) · [数据源](#-数据源) · [项目结构](#-项目结构v3x-架构) · [FAQ](#-faq) · [入群交流测试](#-测试交流群) · [Contributors](CONTRIBUTORS.md)
...
| 你用的 agent | 直接丢这句 |
|---|---|
| **Claude Code** | `/plugin marketplace add heheshang/uzi-skill` 然后 `/plugin install stock-deep-analyzer@uzi-skill` |
| **Codex / OpenAI CLI** | "按 https://raw.githubusercontent.com/heheshang/uzi-skill/main/.codex/INSTALL.md 装 UZI-Skill，分析 600519" |
...
> - **Tier-1 五方法**（v3.8）：`--method ai-readiness` / `earnings-preview` / `model-update` / `returns` / `rebalance`
...
> - **架构**：Rust 单二进制（`crates/` 工作区）· 两段式 Stage 1 / Stage 2 · golden 差分测试对照上游 Python 行为
>
> **Hermes 用户**：Hub 安装若被上游 Skills Guard 误判，可改用源码编译（`cargo build --release -p uzi-cli`），或从 GitHub 提 issue 反馈。
...
> 请按照 https://raw.githubusercontent.com/heheshang/uzi-skill/main/.codex/INSTALL.md 的指引安装 UZI-Skill，然后帮我深度分析 贵州茅台。

### OpenClaw / 龙虾

对龙虾说：

> 帮我安装 https://github.com/heheshang/uzi-skill 这个股票分析技能，装好后分析 贵州茅台。
...
> 请按照 https://raw.githubusercontent.com/heheshang/uzi-skill/main/.opencode/INSTALL.md 安装并分析 贵州茅台。

### Windsurf / Devin / 其他 Agent

丢这句话进去：

> 克隆 https://github.com/heheshang/uzi-skill ，读 AGENTS.md 了解怎么用，帮我深度分析 贵州茅台。
...
```bash
uzi 600519.SH --depth lite --no-browser            # 30-60s 快速档
uzi 300394.SZ --school I                           # 只看 Serenity 卡位视角（A-I 九派任选）
uzi --versus 600519.SH 000858.SZ 002594.SZ         # 2-4 只票横向对决 · ★WIN 高亮
uzi --portfolio holdings.csv                       # CSV 组合 · 加权评分 + 健康度
uzi 600519.SH --output-dir /tmp/out                # SaaS 集成 · index.html + meta.json
uzi 600519.SH --remote                             # 公网链接 · 缺 cloudflared 时默认不改系统
uzi --screen daily --mode close --snapshot-only --no-browser   # 每日榜 · 只用行情横截面
uzi --preview                                      # 内置 mock 数据离线出报告，验证模板
...
先跑一次 `--stage1` 把缓存和机构建模建好（所有 `--method` 都依赖它），再用 `--method` 单独取某一个方法的结果：
**stdout 是纯 JSON**，可以直接 `jq`，也方便喂给下游 SaaS / 脚本。

```bash
uzi <ticker> --stage1              # 前提：采集 + 建模 + 骨架分
uzi <ticker> --method dcf          # 单个方法结果 → stdout 纯 JSON
...
| `--method` NAME | 类型 | 读什么 |
|---|---|---|
| `dcf` · `comps` · `lbo` · `three-statement` | 缓存型（dim 20 估值建模） | `raw_data.json → dimensions.20_*` |
| `initiate` · `earnings` · `catalysts` · `thesis` · `morning-note` · `idea-screen` · `sector-overview` | 缓存型（dim 21 研究工作流） | `dimensions.21_*` |
| `ic-memo` · `dd` · `competitive` · `unit-economics` · `value-creation` | 缓存型（dim 22 深度决策） | `dimensions.22_*` |
| `ai-readiness` · `earnings-preview` · `model-update` | 按需计算（Tier-1） | `uzi-models::tier1` 现场算 |
| `rebalance` · `returns` | 组合 | 配 `--portfolio <csv>` 使用 |

**缓存型**只是读取已经算好的建模结果；**按需计算**现场算。完整 NAME 清单见
[`commands/`](commands/) 与 [`skills/deep-analysis/SKILL.md`](skills/deep-analysis/SKILL.md)。

### 🔀 两段式：`--stage1` / `--stage2`

流水线中段**必须由 agent 介入**（role-play 66 位评委、审查假设），所以常规流程拆成两段：

```bash
uzi <ticker> --stage1              # Task 1→3：采集 + 机构建模 + 规则引擎骨架分，停下等你
# → 读缓存 → role-play 66 位评委 → 写 .cache/<ticker>/agent_analysis.json
uzi <ticker> --stage2              # Task 4→5：合并 agent_analysis.json → 综合研判 → 出报告
```

`uzi <ticker>`（不带 `--stage*`）等于一把跑完 Task 1→5，评委为规则引擎的机械输出，
适合 lite/medium 的快速场景；`--depth deep` **不会**一把梭，必须走 `--stage1 → 写 agent_analysis.json → --stage2`。

### 📐 分业务建模：`--segmental`

```bash
uzi 300308.SZ --segmental discover   # 生成分业务收入骨架（3 情景 × 3 年 projection）
uzi 300308.SZ --segmental validate   # 对账校验：bottom-up 与 top-down DCF 是否对得上
```

### 🛡 机械自查：`--stage-review`

```bash
uzi <ticker> --stage-review          # 跑 17 条 self-review：exit 1=critical · 2=warning · 0=通过
```

报告生成前会自动跑同一套检查；`critical > 0` 时物理上出不了 HTML，必须修完再出。

### A+港股每日观察榜

`uzi --screen daily` 是全市场扫描，不等同于单只股票的 `/stock-deep-analyzer:screen <ticker>`。默认抓取 A 股和港股实时横截面，过滤 ST、退市整理、无有效报价和实际成交额低于 2 亿本币的标的，再生成规则初筛观察结果。默认增强行业、结构化消息、盘口及分钟线；当前仍不是 Agent role-play，也不保证实际成交，不会单凭高分给出执行信号。
...
uzi --screen daily --mode noon --snapshot-only --no-browser  # 网络受限时只用行情横截面
```

- `--top 10` 是上限，不是配额；低于 70 分的研究候选不会为凑数入榜。
- `--mode noon/close` 不会把实时行情变成历史快照。报告记录实际观测时间；下午运行会显示下午时点，不会回写成上午。历史调用必须显式提供冻结快照，并禁止混入实时消息。
...
- 输出位于 `reports/`（`UZI_REPORTS_ROOT` 可覆盖），包含 `picks.json`、`report.meta.json`、`index.html`。
- 账本追加到 `.cache/_daily_screen/signals.jsonl`。规则分未经概率校准，不是胜率；账本当前只记录候选，尚未完成收益追踪和样本外验证。

deep 档另有强制 role-play 门禁：`uzi <ticker> --depth deep` 只执行 Stage 1。Agent 必须读取当前股票缓存中的
`_agent_review_context.json`，把 `analysis_input_hash` 写入 `agent_analysis.json` 后才能调用 Stage 2；旧快照的角色判断不会被复用。
...
Rust 版把这条校准逐字节复刻进 `uzi-pipeline` 的评分纯函数，并由 golden 差分测试锁定。
...
```bash
uzi 600519 --depth lite     # ⚡ 速判模式（1-2 分钟）
uzi 600519                   # 📊 标准分析（5-8 分钟）· 默认
uzi 600519 --depth deep      # 🔬 深度研究（15-20 分钟）
```
...
| **CDP 浏览器兜底**（v2.13.1） | ❌ 完全禁用 | opt-in · `UZI_PLAYWRIGHT_ENABLE=1` · **6 维**（4_peers/8_materials/15_events/17_sentiment/7_industry/14_moat） | ✅ 默认启用 · **10 维**（medium 6 + 3_macro/13_policy/18_trap/19_contests）· 首次 y/n 交互装 Chromium |
...
每个人除了量化规则，还有自己的**真实语录库**（`skills/investor-panel/references/quotes-knowledge-base.md`，45+ 位、
每条带出处 URL）和**决策画像**（时间框架 / 仓位风格 / 什么会改变我的看法）。
...
51 份 persona 档案在 [`skills/deep-analysis/personas/`](skills/deep-analysis/personas/)（12 flagship 手写 + 39 stub），
是**功能性资产**：`uzi` 从仓库根目录运行时默认加载，agent role-play 时逐份读取。
...
> ⚠️ 身份与收益均为**自述/媒体转述、未经第三方审计**，各来源数字互相矛盾。本项目仅蒸馏其**方法论**作为一个分析视角，不代表认可其真实战绩。
...
```bash
uzi 300394.SZ --school I       # 只看 Serenity 的"卡没卡位"判断
uzi NVDA --school H            # 只看 H 组科技领袖派（黄仁勋/Musk/Altman/Saylor）
```
...
单独取某一个方法的结果：`uzi <ticker> --method <NAME>`（stdout 纯 JSON）。
...
报告产物落在 `reports/{ticker}_{YYYYMMDD}/`：`full-report.html`（自包含，离线可看）、`full-report-standalone.html`、
`one-liner.txt`、`avatars/`。

---

## 🔧 数据源

全部免费，零 API key。**本项目没有任何 Python 依赖，也没有 akshare**——所有数据都由二进制内的 HTTP 客户端直连端点抓取，
按 `A → B → C → D` 三级降级链切换，字段级来源清单见 [`skills/deep-analysis/references/data-sources.md`](skills/deep-analysis/references/data-sources.md)。

| 数据 | 主源 | 备用 |
|---|---|---|
| 实时行情 / PE / 市值 | 东方财富 push2（单股 / 全表） | 腾讯 qt → 新浪 hq → 内置行业映射 |
| 财报历史 | 东财 F10 datacenter（主要财务指标） | 港股东财 F10 / 美股 Yahoo fundamentals-timeseries |
| K 线 / 技术指标 | 东财 push2his 日线 | 新浪 getKLineData → 腾讯 fqkline（6 路 fallback） |
| 龙虎榜 / 北向 / 两融 | 东财 datacenter | 交易所披露（深交所 / 上交所） |
| 研报 / 公告 | 东财研报 + 巨潮 cninfo hisAnnouncement | 同花顺 / 财联社 / 金十快讯 |
| 港股 | 东财港股 spot（push2 `m:116`）+ 腾讯 qt hk | Yahoo chart v8 |
| 美股 | Yahoo chart v8 + fundamentals-timeseries | 东财 |
...
多层 fallback 链 — 一个源挂了自动切下一个。每条数据都带 `source` 字段；走 web search 兜底时标 `fallback: true`，
报告前端显示"[网络搜索]"徽章而非"[官方接口]"。
...
配置：
```bash
export MX_APIKEY=<你的 key>        # 免费申领：https://dl.dfcfs.com/m/itc4
```

无 key 时全部回退到腾讯 / 新浪 / 东财 HTTP 链，现有用户零感知。
...
| 数据源 | 维度 | 启用方式 | 影响 |
|---|---|---|---|
| **XueQiu 实盘组合** | `19_contests` 实盘比赛持仓 | `export UZI_XQ_LOGIN=1` 然后跑 `uzi --xueqiu-login`（一次性弹浏览器登录） | 不启用：报告 19_contests 显示"⚠️ XueQiu 需登录，0 cube"；启用后能看到雪球 50+ 个实盘组合持有本股 |
...
若某些字段拿不到（网络限制 / 新股 / 停牌），pipeline **不会塞默认值糊弄**：

1. 生成 `_data_gaps.json` 列出每个缺口的建议恢复动作（CDP 浏览器 / MX / WebSearch / 推导）
...
本项目**没有安装步骤、也没有运行时依赖**——单一二进制直接跑，数据源全部走 HTTP 直连。
主源被拦截时按 [`references/data-sources.md`](skills/deep-analysis/references/data-sources.md) 的三级降级链自动切换，
不需要配镜像、不需要重装任何东西。
...
但机构建模或评分阶段中断，使用 `uzi <ticker> --from-modeling` 从
`raw_data.json` 继续；如果 `dimensions.json` 和 `panel.json` 也已生成，可直接跑 Stage 2，
都不需要从头抓取。
...
> 本仓库是上游 Python 版的 **Rust 重实现**：同样的 22 维数据、同样的 242 条评委规则、
同样的评分公式与报告结构，但业务逻辑全部编译进单一二进制 `uzi`，运行时零外部依赖。

```
UZI-Skill/
├── Cargo.toml / Cargo.lock             # Rust workspace
├── SKILL.md                            # 根索引（skill 路由 + 命令速查）
├── crates/                             # ← 全部业务代码（10 个 crate）
│   ├── uzi-core/                       # 通用：缓存 / py 兼容语义 / 测试工具 / pyrandom
│   ├── uzi-data/                       # 22 维采集 + HTTP 端点 registry + 新闻/热榜/浏览器
│   ├── uzi-features/                   # 108 标准化特征 + 情景模拟（friendly）
│   ├── uzi-investors/                  # 66 评委 × 242 规则 + persona YAML + 席位库
│   ├── uzi-models/                     # 机构建模（DCF/Comps/LBO/IC Memo…）+ Tier-1
│   ├── uzi-pipeline/                   # 采集 → 评分 → 合成 主干
│   ├── uzi-report/                     # HTML 报告装配（模板 / SVG / 维度卡 / 面板卡）
│   ├── uzi-review/                     # 17 条机械自查 + 数据完整性 + schema 校验
│   ├── uzi-screen/                     # A+港股每日观察榜
│   └── uzi-cli/                        # 二进制入口 `uzi` + 参数解析
├── skills/
│   ├── deep-analysis/                  # ★ 主 skill（个股分析）
│   │   ├── SKILL.md
│   │   ├── personas/*.yaml             # 51 份 persona（12 flagship + 39 stub）
│   │   └── references/                 # 方法论：数据源 / 打分 / 建模 / 报告 / fin-methods
│   ├── investor-panel/                 # 评审团 skill（含 quotes-knowledge-base.md）
│   ├── lhb-analyzer/                   # 龙虎榜 skill
│   └── trap-detector/                  # 杀猪盘 skill
├── commands/*.md                       # 20 张命令卡（逐方法操作步骤）
...
| 上游 Python 实现 | 本项目（Rust） |
|---|---|
| CLI 入口脚本 | `uzi` 二进制（`crates/uzi-cli`） |
| 采集 / 评分 / 合成管道（collect · score · synthesize） | `crates/uzi-pipeline` |
| 22 维数据采集层 | `crates/uzi-data` + `crates/uzi-features` |
| 66 人 × 242 规则评委引擎 | `crates/uzi-investors` |
| 机构建模 + Tier-1 方法 | `crates/uzi-models` |
| 报告装配层（模板 / SVG / 卡片） | `crates/uzi-report` |
| 机械自查 gate（13 条 → 17 条） | `crates/uzi-review`（`--stage-review`） |
| 全市场扫描脚本 | `crates/uzi-screen`（`--screen daily`） |
| Python 依赖清单 / 虚拟环境 | 无 —— 依赖全部静态编译进单一二进制 |
| pytest 回归 | `cargo test` + [`tools/golden/`](tools/golden/README.md) 逐字节差分 |

**行为对齐**：`tools/golden/` 喂同一份输入，逐字节比较 Rust 与上游 Python 的输出树
（key 顺序、浮点精确相等、数组长度）。唯一刻意保留的非确定性是评委台词首句的随机抽取，
Rust 端改为确定性选择，并通过 `persona_pools.json` 校验其必属上游可能产出的集合。
...
```
Stage 1 (uzi)            → 数据采集 + 模型计算 + 规则引擎骨架分
        ⏸️ Agent 介入     → 读数据 → role-play 66 评委 → 写判断 → 审查假设
Stage 2 (uzi)            → 综合研判 + 自动跑 17 条自查 → 报告生成
                           ↑ v2.9 核心：critical 不过 → 拒绝出 HTML
```

```
          user says "/analyze-stock BABA"          （或 uzi BABA --stage1）
                       ↓
   ┌──────────────────────────────────────────────┐
   │   Task 1 · Stage 1 — 并发数据采集             │
   │   22 维 fetcher × 数十个 HTTP 源              │
   │   (东财 / 腾讯 / 新浪 / cninfo / Yahoo / DDG) │
   └──────────────────────────────────────────────┘
                       ↓ raw_data.json
   ┌──────────────────────────────────────────────┐
   │   Task 2 · 规则引擎打分                       │
   │   22 维 → dimensions.json                    │
   │   66 评委 × 242 规则 → panel.json             │
   └──────────────────────────────────────────────┘
                       ↓ HARD-GATE（agent 接管）
   ┌──────────────────────────────────────────────┐
   │   Task 3 · Agent 分析                        │
   │   读 quotes-knowledge-base.md + persona YAML │
   │   写 agent_analysis.json                     │
   ...
   │   uzi-review — 17 条自动检查                  │
   │   critical > 0 → 拒绝出 HTML                  │
   │   agent 修完 → 重跑 review → 通过             │
   └──────────────────────────────────────────────┘
                       ↓ only when passed
   ┌──────────────────────────────────────────────┐
   │   Task 5 · 报告装配                          │
   │   → full-report.html（单文件，离线可看）      │
...
**`uzi-review`** 的 17 条自动检查覆盖所有历史 BUG 经验：
...
报告装配入口自动跑 review，critical > 0 → 抛错并**物理上无法出报告**，直到 agent 修完。
...
每次新 BUG 修完，对应的 `check_*` 规则都会加到 `uzi-review`，**下次同类问题跑完就自动抓到，不再靠用户反馈**。
...
**Q: 提示词里说的 akshare 抓不到怎么办？**
A: **本项目不用 akshare**——上游 Python 版用它做封装，Rust 版全部改成直连 HTTP 端点（东财 / 腾讯 / 新浪 / cninfo / Yahoo）。某个端点拿不到时按三级降级链自动切换，完整链条见 [`skills/deep-analysis/references/data-sources.md`](skills/deep-analysis/references/data-sources.md)。

**Q: 能当投资建议吗？**
A: 不能。这是工具不是神仙，66 个大佬的意见都是规则引擎模拟的，不代表真人观点。买不买你自己决定。

**Q: 怎么知道这次报告数据是否可信？**
A: v2.9 起**强制**机械自查。报告生成前跑 17 条检查，critical 不过物理上发不出报告。`.cache/<ticker>/_review_issues.json` 里能看到本次跑有没有 warning，每条都带 `suggested_fix`。也可以手动 `uzi <ticker> --stage-review`（exit 1=critical / 2=warning / 0=通过）。每次新 BUG 修完都加对应检查 → 下次同类问题自动抓到，不靠用户反馈。
...
- 选"否"→ 下次启动再问
- 网络慢 / 关掉检查：`export UZI_NO_UPDATE_CHECK=1`（CI / Codex 环境推荐）；也可以 `uzi --check-update` 只检查一次
- 缓存 6 小时 · 不会每次都打 GitHub API

**Q: 之前报告里有 BUG 怎么办？**
A: 2026-04-17 前跑过"工业金属 / 工业母机 / 工业机械"相关股票的用户，cache 里的 `7_industry` 维度是错的（云铝被归为农副食品加工的那个 bug）。清掉 `.cache/<ticker>/` 后重跑 `uzi <ticker>` 即可。

---

## 📋 更新日志

> 本项目与上游 Python 版**共用同一套版本号**，逐版对齐功能行为，因此下表沿用上游的版本史。
> 两点说明：
>
> 1. **条目里的人数/维度数反映该版本当时的状态**（如 v1.0 的 "19 维 + 50 评委"、v2.13.3 的
>    "51 评委"），是历史记录，**不是当前值**；当前值见本文档前面的章节（22 维 / 66 位 / 9 组）。
> 2. 早期条目提到的 Python 模块与函数名（如 `gen_pixel_avatars` / `render_school_scores` /
>    `STYLE_GROUP_WEIGHTS`）是**上游旧实现**的位置。在本 Rust 仓库里对应实现分别在
>    `uzi_report::avatars` / `uzi_report::special_cards` / `uzi_features::stock_style`。
>    功能行为已对齐（由 `tools/golden/` 的差分测试保证），只是承载它们的模块不同。
>
> 想知道"上游模块 → 本项目模块"的完整对应，见 `AGENTS.md` 的映射表。

| 版本 | 日期 | 主要变化 |
|---|---|---|
| **v3.9.4** | 2026-08-08 | **前端显示修复 + 美化（kimi-k3）** · ① None 泄漏清零：政策面（方向/补贴/监管/反垄断）与资金面"主力资金 20日"在数据源返回 null 时曾直接显示 `None`，现统一兜底为 "—"/"数据暂缺"。② 移动端图表裁切：sparkline SVG 固定 220px 宽度撑破容器（LBO 的 EBITDA 路径/债务偿还、维度卡迷你图），改为 `width:100%` 自适应 + 移动端单列堆叠；敏感性表/机构建模表移动端横向可滚动。③ 治理面措辞："关联交易/违规"此前仅因搜过查询词就显示"未发现"，改为中性"暂无公开违规记录"。④ 抄作业卡片标签与正文粘连，改为标签不换行+留白+正文右对齐。⑤ 美化：维度卡顶部常驻分级色条（高绿/中金/低红）+ hover 高亮、全局平滑锚点滚动、文本选择色、`:focus-visible` 聚焦环、细滚动条、section/卡片进入视口轻浮现（尊重 `prefers-reduced-motion`）、TOC 锚点 `scroll-margin` 防遮挡。⑥ 版本对齐：3 个子 skill（investor-panel/lhb-analyzer/trap-detector）原停在 3.9.2 与主 manifest 不一致，全部统一 bump 到 3.9.4。全量回归通过 |
| **Unreleased** | 2026-08-05 | **全球同行业绩对比 + 数据完整性 hotfix** · 自动按全球细分行业发现候选并补全 Top 8 年度财务，覆盖日/韩/台/新/印/加/澳/英/欧洲等 Yahoo 交易所后缀；原币与 USD 标准化值分离，跨币种原始市值不混算，同行失败隔离、后备候选递补并缓存 24h。报告新增目标高亮、同行中位数、全球分位、规模/毛利率散点和明细表，结果接入 Comps 与维度评分。同步包含 issue #87/#90 的行情 scale、自身同行剔除和美股 TTM 修复。22 个全球同行专项测试 · 全量回归通过 |
| **v3.9.3** | 2026-08-07 | **茅台深度实测驱动的数据/渲染修复** · ① 现金流误判：`fcf_positive` 从 OCF/净利比改为真实 FCF，茅台(FCF 658亿)不再显示"现金流为负"；`_ic_risks` 的"行业周期下行"不再无条件追加，仅行业判定衰退时才算。② 同行修复：push2 被反爬时 4_peers 曾只剩自己一行("暂无可比股")——新增 INDUSTRY_PEERS 硬编码同行兜底(白酒→五粮液/泸州老窖/洋河/山西汾酒)，用东财 F10 指标(不走 push2)拉 ROE。③ 渲染修复：评委 pass/fail dict 不再泄漏成内部字面量；4_peers 对比条 `abs(None)` 崩溃；一致目标价不再显示"(None)"。④ deep 档强制 agent 介入 role-play：`--depth deep` 不再 CLI 一把梭，须 Stage 1 → 读 persona → 写 agent_analysis.json(`per_investor_override` 已能真正合并)。⑤ Codex 评审反馈(P1/P2)：`fcf_positive` 优先用真实经营现金流；同行兜底补别名匹配(集成电路→半导体 / 工业金属→有色金属)且被分析股不在同行列表时补 self 行；HK rank-only 的 `peer="—"` 不再被渲染成 0 对比条。45+ 回归测试 |
| **v3.9.2** | 2026-07-07 | **流程与数据契约 hotfix（issue #82/#83）** · ① 财报采集显式输出 `ocf` / `ocf_history` / `ocf_to_net_income_ratio`，不再只把经营现金流藏在 `fcf` 字段里；`uzi-features` 读入 OCF/净利比，避免 trap-detector 默认 1.0 误判。② `industry=None` 时同行采集返回 self-only fallback + reason，估值用 cninfo 市场加权 PE 兜底并标明原因。③ 采集 registry 字段契约对齐旧输出（`financial_health` / `pe_quantile` 等），避免假 data_gaps。④ `agent_analysis.json` 结构性 schema error 现在真正 fallback 到脚本骨架，不再污染 synthesis。⑤ CLI 统一 fund summary / `--versus` / `--portfolio` 的浏览器、`--output-dir`、`--remote` 后处理；`cloudflared` 缺失时默认不自动安装，需显式 `--install-cloudflared`。8 个新回归测试 |
| **v3.9.1** | 2026-06-23 | **HTML 报告导航栏可折叠（issue #79 · @QKioi）** · v3.6.0 加的左侧 sticky 章节导航栏会略微遮挡正文，本次按社区建议加折叠按钮：展开态 `◀`，点一下收起成一个 `☰` 小把手（items 全藏 · 不再压住正文），再点一下展开 · 状态写入 `localStorage`(`uzi-toc-collapsed`) 刷新记忆 · 全程安全 DOM(无 innerHTML) + `aria-expanded` 可访问性。7 个新回归测试 |
| **v3.9.0** | 2026-06-11 | **新评委「股海贼王」· 首位从真实交割单蒸馏的评委 (65→66)** · 数据源：淘股吧十年实盘帖 (2016-02 开贴) · 3898 张持仓截图 OCR → **8951 笔反推交割单** + **5069 条发言**。定量画像：33 万→3131 万 (~95 倍/10 年) · 持仓中位 1 天/P75 3 天 · 同时 3-5 只 · 第一重仓中位 51% · 10 年 2010 只票题材轮动 (鸿博/川能/人民网/大众交通)。方法论蒸馏（风格提炼·不逐字转载原帖）：复盘三问(为啥涨停/板块地位/大盘地位) · 弱转强快速板才算超预期 · 逻辑硬的低位票爆发力足 · 格局票=时代的情绪载体(三五倍格局论) · 反复强调不跟单。落地：F 组 flagship · 6 条数据驱动规则 (阈值来自其真实行为统计) · 台词按风格原创撰写。**原始交割单/截图/发言均为本地数据 · 未入库。** 实测：鸿博式妖股 bullish 100 (他真做过 22 次) · 茅台 bearish 9.5 · 美股 skip。10 个新回归测试 |
| **v3.8.1** | 2026-06-09 | **skills 全面体检 · H/I 两组配套层 6 处补齐** · 体检发现 v3.6.3/v3.7.0 加 14 位评委时配套层漏更新（全部静默降级所以没暴露）：① 14 评委缺头像 SVG → 报告破图（gen_pixel_avatars 补齐 · 总 66）② `render_school_scores` order=[A..G] → H/I 两派分数永远不渲染 ③ GROUP_LABELS 缺 H/I → 显示裸字母 ④ `GROUP_DEFAULT` 缺 H/I → profile 全 "—" ⑤ `STYLE_GROUP_WEIGHTS` 缺 H/I → v2.7 风格加权对两组失效（现 I 在红利股 0.2/科技成长 1.5）⑥ 13 新评委补显式 MARKET_SCOPE + PERSONAS voice 台词。文档同步 ~35 处（52→65 评委 / 7→9 流派 / 180→242 规则 / `--school A-G`→A-I）。10 个新体检回归 |
| **v3.8.0** | 2026-06-08 | **Tier-1 五方法 + Serenity 严谨化 + 技术指标/杜邦扩展**（参考 `anthropics/financial-services` + `lolifamily/ashare-mcp` + `muxuuu/serenity-skill`）。**① Tier-1 5 方法**（`uzi-models::tier1`）：`ai-readiness`（AI 就绪度/卡位）· `earnings-preview`（财报前预览 · Bull/Base/Bear + 隐含波动）· `model-update`（增量更新模型 · 假设 delta + 对 DCF/Comps/thesis 影响）· `returns`（组合收益归因）· `rebalance`（逐持仓再平衡 + 本地化换手成本，A股无资本利得税故不做 TLH）。**② Serenity 严谨化**（不再只会看多）：8 罚分因子（炒作无订单/微盘流动性/杀猪盘/治理/周期/替代设计/地缘/稀释 · 封顶 60% 折扣）+ 3 级证据阶梯（强公告财报×1.0 > 中媒体卖方×0.85 > 弱叙事×0.70 · 同卡位"有定点量产"≈90 vs"仅题材"≈60）+ 供应链 8 层分层（材料→…→下游 · 越上游瓶颈分越高）。**③ 指标扩展**：DuPont 杜邦分解（ROE 质量来源 margin_driven/leverage_driven · 价值派用）+ KDJ/OBV/Williams%R（K 线卡片新增副指标徽章 · 技术派用）。61 个新回归测试 |
| **v3.7.1** | 2026-06-04 | **首页补 Serenity 介绍 + `--school H/I` 放开** · 用户反馈"README 没把 Serenity 写清楚"。(1) 评审团章节重写 52→65 人 · 7→9 组完整表 (A–I)· 规则 180→242; (2) 新增专门 `## 🧠 I 组 · Serenity` 介绍块（她是谁 + 未审计免责 + Chokepoint Theory + "卡位决定态度"打分表 + `--school I/H` 用法）; (3) **bug fix** · v3.7.0 起 evaluator 已含 H/I 但 CLI `--school` 的 choices 还停在 A-G · 文档说能用实跑报错 · 现扩到 A-I + 补中文流派名; (4) README/AGENTS 当前状态计数同步。全量回归通过 |
| **v3.7.0** | 2026-06-03 | **13 位新晋科技大佬集体入团 · 52→65 评委** · 用户反馈"评委库新晋科技 / AI / VC 视角覆盖不足"。新增分布：**B 成长派 +5** (Marc Andreessen a16z / Bill Gurley Benchmark / Naval Ravikant AngelList / Brad Gerstner Altimeter / Chamath Palihapitiya Social Capital) · **C 宏观派 +2** 做空猎手 (Michael Burry Big Short / Jim Chanos Kynikos) · **E 中国价投 +1** (张磊 高瓴 · "做时间的朋友") · **G 量化派 +1** (Cliff Asness AQR 价值×质量×动量三因子) · **H AI 卡位/瓶颈猎手 +4** (黄仁勋 NVIDIA / Musk TSLA / Sam Altman OpenAI / Saylor MSTR)。每位评委 ≥4 条规则 (测试守护) · SCHOOL LOCK banner 全派代表评委更新为真实在册成员 · H 派配色紫色 🔗。NVDA 跑分实测 Andreessen/Gerstner/Huang/Altman 全 100 · 茅台跑分 Andreessen 38 (industry filter ✓ 白酒不在 software/AI) · 张磊 80 (✓ 长跑道龙头)。18 个新回归测试 |
| **v3.6.3** | 2026-06-03 | **重磅角色 Serenity · AI 卡位/瓶颈猎手（新增第 52 位评委 · 独立 I 组）** · 把 X 爆火的 AI 供应链「卡脖子/瓶颈点」投资人 Serenity ([@aleabitoreddit](https://x.com/aleabitoreddit)) 接入评审团 · 参考 [serenity-alpha skill](https://github.com/haskaomni/serenity-skill/tree/main/serenity-alpha) 方法论 + 爬 X 真实言论建语气库。**核心打分逻辑「卡位决定态度」**：新增派生特征 `ai_chokepoint_score`（AI 链命中 × 不可替代性 × 中小市值弹性 × 需求拐点）· Serenity 规则全 weight 5 · 产品卡在 AI 瓶颈（光模块/CPO/HBM/CoWoS/InP 衬底…）+ 上游不可替代 + 小市值 → **bullish 重仓**；没卡到位/不在 AI 链上 → **bearish 不碰**（白酒/银行护城河满分也 score=0）；在链但卡位不硬 → neutral 待验证。改动覆盖评委库 / 规则引擎 / 席位视角 / persona / 派生特征 / 报告卡片。新增 `group-i-serenity.md` + fin-methods 文档 |
| **v3.6.2** | 2026-06-03 | **cninfo 翻页长尾修复 + Hermes 安装脚本（上游 Python 版本特有）** · 两个社群 issue 同发。**#68** ([@xy2yp](https://github.com/wbh604/UZI-Skill/issues/68))：`--versus 000958 600406 --depth lite` 卡在 15_events cninfo 公告（上游封装要翻完全部 854 页才返回，`.head(30)` 是事后截取没用）。修法：改为**直连 cninfo `/new/hisAnnouncement/query` HTTP** · `pageSize=30 + pageNum=1 + 15s 硬超时` · 几小时 → ≤15s。Rust 版从一开始就走这条直连快路径。**#69** ([@FrankHuy](https://github.com/wbh604/UZI-Skill/issues/69))：上游 Linux 安装脚本探测 pip 失败——Rust 版无安装脚本、无 pip，天然不存在此问题 |
| **v3.6.1** | 2026-05-29 | **Hermes Skills Guard 假阳性绕过（上游封装层）** · [issue #66](https://github.com/wbh604/UZI-Skill/issues/66) (@zodiacg) 反馈：`hermes skills install` 报 `Verdict: DANGEROUS · 168 findings` · `--force` 也覆盖不了。诊断：Hermes Skills Guard 模式匹配假阳性（把读环境变量当 "exfiltration"、把 `curl \| bash` 安装当 "privilege_escalation"、HTML 注释当 "injection"），Hermes 团队已知问题 ([#1006](https://github.com/NousResearch/hermes-agent/issues/1006))。上游当时用一键脚本绕过；本项目推荐改用源码编译（`cargo build --release -p uzi-cli`）或 GitHub issue 反馈 |
| **v3.6.0** | 2026-05-29 | **视觉/交互大升级 + 多股横向对比 + 组合分析** · 三 Phase 拼合发布。**Phase A 视觉**：(1) 暗色模式 toggle（右上角 🌙 · `localStorage` 持久化 + `prefers-color-scheme` 自动初始化）· (2) 左侧 sticky TOC + IntersectionObserver scroll-spy · (3) 大评分 count-up 动画 · (4) PE/PB/ROE/DCF/IRR/WACC/EV-EBITDA/LBO/YTD/TTM/PEG/LHB 自动包 `.jargon` + 悬浮 tooltip · (5) 报告底部 QR 码（扫码直达完整报告）· 🔒 全部用 `createElement + textContent` 安全 DOM · 不用 innerHTML (XSS 防御)。**Phase B `--versus`**：2-4 只票横向对比 · ★ WIN 高亮 12 个核心指标 · 单 HTML 自包含 · 复用 cache。**Phase C `--portfolio`**：用户上传 CSV (ticker/weight/note) · 容错解析（header/无 header/中英文列名/0-1 vs 0-100 权重）· 权重自动归一 · 输出排名 + KPI（加权评分 + 集中度 + 行业分散）+ metadata.json |
| **v3.5.0** | 2026-05-29 | **单一流派视角锁定 (`--school A-I`) + SaaS 集成 (`--output-dir`)** · 社群反馈"我只想看 F 派游资视角 · 不想 66 评委一起 vote"。(1) CLI 加 `--school A/B/C/D/E/F/G/H/I` · 价值/成长/宏观/技术/中国价投/游资/量化/科技领袖/Serenity 九选一 · 非该派评委直接 skip 不进规则引擎。(2) synthesis 编码 `school_lock={group,label}` · 报告顶部渲染 SCHOOL LOCK banner（各派配色，A=深绿/F=深红/G=青），让分享者一眼看出本次仅看了该派 · 避免被误读为 66 评委结论。(3) SKILL.md 加 HARD-GATE · agent role-play 时严格只 role-play 该派 · 不写跨派对比。(4) 同时合入 `--output-dir DIR` SaaS 集成参数 · 把报告目录拷到外部路径 + 写 `index.html` / `report.meta.json` 供 Celery worker 落库。11 个新回归测试 |
| **v3.4.5** | 2026-05-12 | **F 派游资 LHB 反查 + low-confidence banner** · 社群 codex agent 跑京东方 (000725) 实测发现两点：① F 派 23 人全 skip（市值 2000 亿超射程）· 但 LHB 实际有 3-5 个游资席位参与涨停博弈 · 评委逻辑与数据脱节。② 规则引擎 fund_score 37.6 但 agent 重评 65/100 · 报告无任何"score 不可信"警告 · 用户误判。修法：(1) 游资射程检查加 LHB 反查 · 匹配到该游资昵称时强制不 skip。(2) 数据缺口 banner 新增判断 · stock + fund_score<50 + cov<60% → 渲染 low-confidence 红色 banner · 文案明确引导看 agent 重评估。10 个新回归测试 |
| **v3.4.4** | 2026-05-12 | **data quality banner UX 优化** · 社群反馈两点：① ETF 报告 "覆盖率 17%" 让用户误判可信度（实际 ETF 本就无 ROE/PE 个股字段）· ② 橙色 banner 上橙色字看不清。修法：(1) banner 检测 ETF/LOF/mutual_fund · 切换 `fund-type` 蓝色调 banner · 文案明确"基金类型预期缺字段·不影响可信度"+ 引导看持仓股报告。(2) CSS 对比度大改 · 深棕标题 · 加深正文 + 提高字重。11 个新 CSS + 行为回归测试 |
| **v3.4.3** | 2026-05-12 | **开放式基金分类修复 + 字段级 fallback gate** · (1) [#60 复议](https://github.com/wbh604/UZI-Skill/issues/60) (@SchrodingerBarbatos) · 用户输 110011 易方达优质 → 被错判 convertible_bond early-exit。修法：判 cb 前二次校验基金名称 · 加 `mutual_fund` 类型 · 路由到基金持仓循环 · 005xxx 等股票前缀外的基金也识别。(2) [PR #63](https://github.com/wbh604/UZI-Skill/pull/63) (@Wood Letitia) · 字段级 fallback gate · 修 source-level 整块切换导致 name 永远缺的问题 · 主源拿到 price/PE/PB 但 name 空时自动调备用源补全 · 仅填空不覆盖。全量回归通过 |
| **v3.4.2** | 2026-05-11 | **Windows + Clash TLS 兼容（上游 Python 栈特有的 TLS 后端问题）** · 社群反馈"Clash 国内规则 DIRECT 还是 Schannel · 东方财富彻底不可用"。Rust 版使用编译进二进制的 rustls / 系统根证书，没有 Schannel 后端切换问题；同时保留多源降级（行情、财报缺字段时依次尝试备用端点） |
| **v3.4.1** | 2026-05-11 | **verdict 粒度细化** · 社群反馈"神剑股份(58分) / 博云新材(60分) verdict 都是观望优先 看不出差异"。修法：(1) verdict 50-65 拆三档（观望偏空 50-55 / 观望中性 55-60 / 观望偏多 60-65）· 65-70 加"可以蹲（偏弱）"; (2) verdict label 追加"X 派看多 / Y 派看空"; (3) synthesis 加 `verdict_detail = "基本面 X · 共识 Y"` · 渲染时拼到 verdict 后。改后两只票 verdict 仍同段（55-60）但 detail 让基本面 +2 分差异显式可见。5 个新回归测试 |
| **v3.4.0** | 2026-05-10 | **基金/ETF 持仓循环分析** · ETF/LOF 不再 early-exit · 改为列出前 10 持仓 + 估算耗时 + **二次确认**（y / 数字 / N） · 确认后循环跑个股分析 + 生成 fund-holdings-summary.html。partial failure 容忍 · 环境变量可跳过 prompt（CI/agent）。可转债/指数仍 early-exit。新建基金持仓循环模块。真机：510300 持仓正确拉到（茅台 5.89% top1） |
| **v3.3.4** | 2026-05-10 | **V8/mini_racer 崩溃逃生舱（上游 Python 运行时的历史问题）** ([#61](https://github.com/wbh604/UZI-Skill/issues/61)) · 上游在 macOS 新 Python 下 mini_racer V8 isolate pool 可能双重初始化导致进程级崩溃（`try/except` 抓不到）。当时用 sentinel 文件 + 环境变量做逃生舱。**Rust 版没有嵌入式 JS 运行时**，同类崩溃从根上不存在 |
| **v3.3.3** | 2026-05-06 | **社区 PR 4 合 1**（[#52](https://github.com/wbh604/UZI-Skill/pull/52) / [#54](https://github.com/wbh604/UZI-Skill/pull/54) / [#55](https://github.com/wbh604/UZI-Skill/pull/55) / [#59](https://github.com/wbh604/UZI-Skill/pull/59)） · #52 LHB 接口 "近一月" 失效 → 改 YYYYMMDD 日期循环 + 6 mock test。#55 agent_analysis schema 文档化 12 条校验规则。#54 cherry-pick 雷达图 import · #59 修 3.11 嵌套 f-string 语法错误（**避开了原版的缩进 bug**）· 新增 5 个回归测试 · 全量回归通过 |
| **v3.3.2** | 2026-04-28 | **GitHub issue #50 + #51 hotfix**（社区驱动）· #50 [Stage 2 总是超时](https://github.com/wbh604/UZI-Skill/issues/50)：v3.2 拆分时报告模块漏 import sparkline 渲染函数 → stage2 崩 · 补 import 即修复（致谢 @chenxiang-bj 报告 + agent 诊断）。#51 [XueQiu 登录验证失败](https://github.com/wbh604/UZI-Skill/issues/51)：旧 `/cubes/cubes_search.json` endpoint 已下线 · 改用 `/query/v1/search/cube/stock.json?q={code}`（致谢 @bilieebiliee1-design 报告 + @Kylin824 提供 fix）。3 处同步换 endpoint。5 个新回归测试 |
| **v3.3.1** | 2026-04-28 | **Hermes 兼容回归修复 (hotfix)** · 群友反馈"更新后不支持 hermes 报错"。根因：重构期 main 上从未包含 hermes 兼容文件 · 但 README 仍叫 hermes 用户装 main · 装下来缺文件就崩。修复：从 `hermes-compat` 分支 cherry-pick 5 项核心适配到 main + 入口加双 layout 探测。**hermes 用户重装一次 skill 即恢复** |
| **v3.3.0** | 2026-04-23 | **分支大整合**：唯一未合 feature（v2.10 segmental 渲染层 · 228 行 + 222 CSS）cherry-pick 到 v3.2 架构 · 新建 segmental 报告模块。同时清理 22 个 stale 分支（refactor/* / docs/* / feature/* 都已 superseded）· 仅保留 main + hermes-compat · 单一开发主干 |
| **v3.2.0** | 2026-04-23 | **报告装配层深度拆分 (-80%)**：2964 → 587 行 · 拆 5 个子模块（svg_primitives / dim_viz / institutional / panel_cards / special_cards）。v2.x 所有 API 保持 re-export · 全量测试通过 · e2e 零差异 · 加 `CODEX.md` + `AGENTS.md::Repository Layout` 给 codex 准确入口指引。Rust 版对应 `uzi-report` 的模块划分 |
| **v3.1.0** | 2026-04-23 | **评分主体瘦身 65%**：2105 → 735 行。1228 行纯函数（维度打分 / 生成 panel / 生成 synthesis / 定性补齐）抽到独立评分纯函数层；166 行 preflight/resolve/ETF guard 抽到 `preflight` 模块。rrt 做 re-export 保持完整向后兼容 · 全量测试通过 · 002217 resume 10s 出报告。Rust 版对应 `uzi-pipeline` 的纯函数拆分 |
| **v3.0.0** | 2026-04-23 | **pipeline 架构默认启用**：`uzi <ticker>` 默认走统一 pipeline · pipeline 异常自动 fallback。Phase 6c 解耦：评分段直接调纯函数（不再重复 collect）· 002217 score_from_cache 从 180s → 10.6s。Pipeline 预检 guards (中文名 / ETF / LOF / 可转债 → fallback) |
| **v2.15.5** | 2026-04-23 | **评分公式重校准**：`panel_consensus` 从单一 vote 公式升级为混合公式 `0.65*score_mean + 0.35*vote_weighted` + 极化拉伸 (k=1.3) · 解决"大多数分在一个区间徘徊"问题 · 9 流派 consensus 分歧更清晰 · 002217 F 游资 51→43.7（vote 高估修正）· G 量化 50→59.3（实分低估修正） |
| **v2.15.4** | 2026-04-22 | **按流派打分 school_scores**：9 大流派（A 价值 / B 成长 / C 宏观 / D 技术 / E 中式 / F 游资 / G 量化 / H 科技领袖 / I Serenity）各自产出 consensus/avg_score/verdict · 报告新增"SCHOOL SCORES"卡片 · 同一只票可见不同哲学的分歧（宏观派买入 vs 成长派回避） |
| **v2.15.3** | 2026-04-21 | **性能 hotfix** · 资金面采集每股重抓全 A 大宗/解禁/融资数据集 → 每股 3+ min · 新增 4 个 universe helper + 24h TTL 缓存 · 全股共享 · 二次跑 universe 部分 0.01s（**100+ 倍加速**）· 6 专项测试 |
...
| **v2.15.1** | 2026-04-20 | **报告质量 2 bug hotfix**（实测 300470 发现）· Bug 1: fund-card 一堆 "5Y +0.0%" 假数据 · 修基金行降级逻辑 · Bug 2: 14_moat 护城河被贵州茅台数据污染（搜索对生僻公司返超级股票结果）· 加 `_SUPERSTAR_POLLUTERS` 过滤 · 11 专项测试 |
| **v2.15.0** | 2026-04-20 | **YAML persona 接入 agent role-play**（借鉴 augur · 取长补短）：新增 `personas/` 目录 51 个 YAML 文件（12 flagship 手写 + 39 stub 自动生成）· persona 加载 + prefix-stable system message（prompt cache 省 50-90%）· zh/en 语言开关 · `HARD-GATE-PERSONA-ROLEPLAY` 强制 agent 读 YAML · 双盲测试（3 股 × 5 投资者）显示 YAML 14/15 vs Rules 8/15 方向准确率 · 修复 Rules 4 类"历史立场错位"硬伤 · 14 专项测试 |
...
| **main** | 2026-04-20 | **Segmental Revenue Build-Up 落地**（分业务收入建模模块 + CLI + `/segmental-model` 命令卡）· deep 档 `enable_segmental_model` flag 之前只在 profile 声明，实现缺失——本次从老分支 cherry-pick 3 个新文件，零冲突接入 · 同步新增 `CONTRIBUTORS.md` · 清理 14 个已合入幽灵分支 |
| **v2.13.7** | 2026-04-19 | **16 新源真正接入 fetcher**：v2.13.4/6 登记但未用的 16 源全部接通——新建新闻聚合层（金十/东财快讯/东财公告/同花顺 4 源聚合）接入事件 + 舆情维度；Yahoo Chart v8 直连 HTTP 接入美/港股 K 线链；期货协会源接入政策维度。实测 4/4 新闻源通 · A 股 15_events 密度 3-5 → 10-30 条 · 全量回归通过 |
...
| **v2.13.5** | 2026-04-19 | **NetworkProfile 自适应 + agent HARD-GATE 主动触发 CDP 浏览器**：9 目标 3 组网络预检（domestic/overseas/search）+ 代理检测 + 5min cache · SKILL.md / AGENTS.md 加 `HARD-GATE-PLAYWRIGHT-AUTOFILL` 让 agent 主动 FORCE · 15 专项测试 |
...
| **v2.13.2** | 2026-04-19 | **CDP 浏览器触发逻辑升级 · 数据质量感知 + FORCE flag**：维度质量真空检测 · `UZI_PLAYWRIGHT_FORCE=1` 强制全维兜底 · 8 专项测试 |
| **v2.13.1** | 2026-04-18 | **CDP 浏览器兜底全 10 维覆盖**（开源研究场景扩展）：v2.13.0 保守排除的 5 维（7_industry 百度搜索 / 14_moat 百度百科 / 13_policy 证监会 / 18_trap 小红书 / 19_contests 雪球组合）全部加回 · medium 4→6 维 · deep 5→10 维 · 22 专项测试 |
| **v2.13.0** | 2026-04-18 | **浏览器通用兜底 · 按三档 profile 分级**：lite `off` / medium `opt-in` (4 维) / deep `default` (5 维 · 首次 y/n 自动装 Chromium)。新增浏览器兜底模块 · 抽离垃圾过滤模块 |
| **v2.12.1** | 2026-04-18 | **4 个报告板块空数据/错数据修复**（中际旭创实测驱动）：4_peers 三层 fallback + 雪球 opt-in · 7_industry regex 上下文感知 · core_material 垃圾过滤 · BCG 真实算 market_share + 阈值调整 · 16 专项测试 |
| **v2.12.0** | 2026-04-18 | **6 平台社交热榜聚合**：微博/知乎/百度/抖音/头条/B站 官方 API + 5min 文件缓存 + 单平台失败不影响其他 · `17_sentiment` 维度新增 `hot_trend_mentions` 字段补 DuckDuckGo 盲区 · 参考 jcp/hottrend 设计 · 17 专项测试 |
...
| **v2.10.6** | 2026-04-18 | **Providers 框架实际落地**：v2.10.3 建的 5 provider 链（多数据后端 + direct_http）实际接入 K 线链 · 补齐 · 风格 health CLI |
| **v2.10.5** | 2026-04-18 | **v2.10.4 遗漏补丁**：覆盖率阈值 profile-aware（lite 不再结构性偏低）· CLI 直跑 medium/deep 也出报告 · 基金经理 None 字段兜底 |
...
| **v2.10.3** | 2026-04-18 | **三档思考深度**：`lite` (30s-2min · 7 维 + 10 投资者) / `medium` (2-4min · 默认 · 22 维 + 66 投资者) / `deep` (15-20min · 含 Bull-Bear 辩论 + Segmental) · `--depth` CLI arg · direct_http provider（腾讯/新浪/etnet 直连） |
| **v2.10.0-2** | 2026-04-18 | 首次安装 + Codex 机器耗时/token 优化 · 4 层网络超时保护（代理/GFW 不通快速 fail） · Fund holders 双层策略（Top N full + rest lite） · 首次运行 10-15min → 2-4min |
| **v2.9.x** | 2026-04-17 | **机械级 agent 自查 gate**：17 条自动检查 + 报告装配入口强制 block critical → 修完再出 HTML · 行业动态可信搜索（不再是"—"）· HK industry_pe fallback · consensus 半权公式 |
...
| **v2.7.x** | 2026-04-17 | HK financials 实现（BUG#R7）· HK kline fallback 链（BUG#R8）· wave2 flush（BUG#R9）· 风格动态加权 · 量化结构性识别（top-1<2%）· XueQiu 登录 opt-in · 14 权威数据源 + 可信搜索 |
...
- [Linux.do](https://linux.do/) — 测试社区支持
...
实时 stars：![GitHub Repo stars](https://img.shields.io/github/stars/heheshang/uzi-skill?style=social)
