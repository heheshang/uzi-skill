...
> **关于本文档 · 版本史与当前项目的关系**
>
> 本文是上游 Python 版 `UZI-Skill` 的**版本史**（v1.0.0 → v3.9.4，逐版记录改了什么、为什么改）。
> 本仓库是同一项目的新实现（Rust 重写 · 单一二进制 `uzi`），**与上游共用同一套版本号**并逐版对齐
> 功能行为，因此这篇版本史按原样迁入，供当前读者查阅。
>
> - **历史的数字不是当前值**：条目里的人数 / 维度数 / 流派数都是**该版本当时**的真实值 ——
>   当前为 **22 个维度 / 66 位评委 / 9 大流派**。版本号、日期、历史数字与上游 issue / PR 链接
>   均为原始记录，未作任何改写。
> - **Python 模块是上游旧实现的落点**：早期条目提到的 `run.py`、`run_real_test.py`、
>   `assemble_report.py`、`lib/*.py`、`scripts/tests/*.py` 等，在本仓库里**并不存在**；
>   它们在 Rust 版中的对应位置见 `AGENTS.md` 的「上游模块 → 本仓库落点」映射表
>   （例如 `run_real_test` 的 stage1/stage2 → `crates/uzi-cli/src/stages.rs`，
>   `run_real_test` 的纯函数评分 → `crates/uzi-pipeline/src/score.rs`，
>   `assemble_report` 的渲染拆分 → `crates/uzi-report/src/`）。
> - **命令已换成本项目的 `uzi` 形式**：上游的 Python 入口、其 stage1/stage2 内联调用、更新检查，
>   分别对应 `uzi <ticker>`、`uzi <ticker> --stage1` / `uzi <ticker> --stage2`、`uzi --check-update`；
>   构建用 `cargo build --release -p uzi-cli`。本项目**零外部依赖** —— 不需要 `pip`、没有
>   `requirements.txt`，浏览器兜底直接用系统已装的 Chromium 系（`uzi --browser-check` 确认）。
> - **标了「上游 Python 版特有」的步骤**（如 `baostock` 升级、Playwright 浏览器安装、
>   上游 provider 诊断子命令）在本项目**没有对应命令**，保留只为不篡改历史，**不是**今天要执行的步骤。
...
- 用户跑 `uzi NVDA --school H` 时 banner 立刻显示 "Serenity / 黄仁勋 / 马斯克 / Sam Altman / Saylor"
...
**症状**：`uzi --versus 000958 600406 --depth lite` 卡在 15_events 维度的 cninfo 公告抓取：
```
0%|          | 0/854 [00:00<?, ?it/s]
...
4. **`pip install` 失败时给可操作提示**（上游 Python 版安装脚本）：版本太低 / 镜像源建议（清华）/ 升级 pip 命令
...
4. 探测 `~/.hermes/venv/bin/pip` 或 `~/.hermes/.venv/bin/pip` · 装 `requirements.txt`（无 venv 兜底用系统 pip）· **上游 Python 版特有**
5. 打印每个 skill 的版本号验证
...
```bash
uzi --versus 600519.SH 000858.SZ                  # 茅台 vs 五粮液
uzi --versus 茅台 五粮液 002594.SZ --depth lite    # 三只票快速对比
```
...
```bash
uzi 300394.SZ --school F --no-browser    # 仅游资视角
uzi 600519.SH --school A --depth deep    # 价值派的深度分析
```
...
```bash
uzi 600519.SH                                              # 单跑（行为不变）
uzi 600519.SH --no-browser --output-dir /var/uzi/<job_id>  # SaaS 集成
```
...
# CLI 直用
cd UZI-Skill && git pull
# 上游 Python 版特有（本项目零外部依赖，无对应步骤）：pip install --upgrade baostock -i https://pypi.org/simple  # v3.4.0 起锁 ≥0.9.1
```
...
**用户升级**：
```bash
# 上游 Python 版特有（本项目零外部依赖，无对应步骤）：pip install --upgrade baostock -i https://pypi.org/simple
# 上游 Python 版特有：pip show baostock | grep Version  # 应显示 0.9.1+
```
...
> **用户反馈** (@dragonforai)：`uzi SEHK.03690 --depth deep` →
...
**Layer 1 · 显式禁用**:
```bash
UZI_DISABLE_MINI_RACER=1 uzi SEHK.03690 --depth deep
```
...
```bash
UZI_FORCE_MINI_RACER=1 uzi ...   # 即使有 sentinel 也启用
```
...
- Repo root layout（`uzi 002217.SZ`）仍正常
...
- ❌ **v3.1.1 · 22 fetcher adapter 内化**：`fetch_*.py` 仍是独立 CLI 工具（`python fetch_basic.py <ticker>`）· 内化会破坏 user contract · 且 22 × 300 行工作量巨大 —— ⚠️ 上游 Python 版特有：本项目无 `fetch_*.py` 入口，对应逻辑内置于 `crates/uzi-data`
...
⚠️ 默认行为变化：
- **之前**：`uzi 300470.SZ` → legacy stage1+stage2（老路径）
- **现在**：`uzi 300470.SZ` → pipeline.run_pipeline（新路径）
- **回滚**：`UZI_LEGACY=1 uzi 300470.SZ` · 强制走老路径 —— ⚠️ 上游 Python 版特有：本项目**只有 pipeline 一条路径**，没有 legacy 开关，该环境变量在本仓库无实现（`uzi 300470.SZ` 本身可跑，但前缀是惰性的）
...
```bash
python3 lib/news_providers.py "" ""                          # 上游 Python 版特有 · 当时实测
# sources_ok: 4/4 · total_hits: 31

python3 -c "from lib.data_sources import _yahoo_v8_chart; print(len(_yahoo_v8_chart('AAPL','1mo')))"      # 上游 Python 版特有
# 22
python3 -c "from lib.data_sources import _yahoo_v8_chart; print(len(_yahoo_v8_chart('0700.HK','1mo')))"   # 上游 Python 版特有
# 21
```
...
用户发现自动判定太保守时可强制重抓：
```bash
UZI_PLAYWRIGHT_FORCE=1 uzi 300308.SZ --depth deep --no-resume
```
...
uzi 300308.SZ --depth deep --no-browser --no-resume
```
...
deep 模式触发 → 检测 playwright + chromium
  ...
  │    ├─ 用户 y → （上游 Python 版特有）pip install (国内镜像 fallback) + playwright install chromium
  │    │    ├─ 成功 → 继续
...
完整登记在 [docs/BUGS-LOG.md v2.11.0 章节](https://github.com/wbh604/UZI-Skill/blob/main/docs/BUGS-LOG.md#v2110-2026-04-18--评分校准--用户反馈驱动)（上游 Python 版的 bug 记录，本仓库未迁入），含"未来改该区域注意事项"防回归清单。
...
**4. Provider 健康诊断 CLI：`python -m lib.providers`（上游 Python 版特有；本项目无 `uzi` 子命令，但同一实现有入口）**

本项目对应入口（`uzi_data::providers::cli`，经 example 转发）：

```bash
cargo run -p uzi-data --example fetch_one -- --providers            # health
...
```
- `python -m lib.providers chain A kline`（上游 Python 版特有）· 看某维度 provider 优先级 —— 等价入口见上
...
- CLI 实测 `python -m lib.providers` + `chain A` 两个子命令 OK（上游 Python 版特有）
...
- 症状：`uzi 600519.SH --depth lite` → coverage 17%（3/18）→ critical → block HTML 生成
...
- 症状：`uzi` 直跑（无 agent 介入）→ gate 必定 critical，阻止 HTML 生成
...
**3. ETF 早退逻辑半成功**
- 症状：`uzi 512400.SH` → stage1 正确识别 ETF 写 `_resolve_error.json`，但 stage2 仍被调用 → `RuntimeError: Stage 2 缺少数据`
...
- `uzi {ticker} --no-resume` — 强制重抓全部 22 fetcher
