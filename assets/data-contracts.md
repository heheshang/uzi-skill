...
### 3.1 加密资产评委结果（`market="C"`）

加密资产仍复用 `panel.json` 的外层结构，但评委结果必须使用加密原生语义：

- `name` / `en` 是当前加密风格角色（货币派、协议基本面派、链上趋势派等），不是股票人物的原始称谓；`investor_id` 保持稳定，仅用于关联配置和排序。
- `pass` / `fail` 不得出现 PE、PB、ROE、EPS、股息、财报、龙虎榜或 A 股席位等股票专属指标；不适用的指标不应被伪造为 `0`。
- `reasoning` / `headline` 应引用可观测的代币经济、网络与市场指标，例如流通率、FDV/市值、NVT、市值排名、30 日涨跌、最大回撤、资金费率、恐慌贪婪指数和网络份额。
- F 组 A 股游资仍按市场范围 `signal="skip"`，不应被改写成加密短线角色；加密短线角色只适用于明确纳入加密市场的评委配置。

示例：`"headline": "看空：评分 31/100；市值排名 #42，NVT 96.0，30日涨跌 -18.0%"`。
